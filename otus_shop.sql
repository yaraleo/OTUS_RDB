...
-- -----------------------------------------------------
DROP TABLE IF EXISTS `otus_shop`.`categories` ;

CREATE TABLE IF NOT EXISTS `otus_shop`.`categories` (
  `id_categ` INT(11) UNSIGNED NOT NULL COMMENT 'Идентификатор категории товара, первичный ключ. Тип INT самый подходящий.',
  `name_categ` VARCHAR(45) NOT NULL COMMENT 'Имя категории. Взят универсальный формат для текста. Длина произвольная.',
  `perent_id_categ` INT(11) NULL COMMENT 'Внешний ключ ссылается на какое-либо занчение из этой же таблицы, которое является родительским в иерархии категорий.',
  PRIMARY KEY (`id_categ`) COMMENT 'Индекс на ключевое поле',
...
CREATE INDEX `parent_id_categ_idx` ON `otus_shop`.`categories` (`id_categ` ASC, `perent_id_categ` ASC) COMMENT 'Индекс на внешний ключ' VISIBLE;
